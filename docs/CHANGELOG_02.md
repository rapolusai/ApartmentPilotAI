...
- Current backend JUnit test/package, Android clean debug build, PostgreSQL/Flyway V4 validation, 50-check local API suite and 112-check operations/concurrency suite passed. Device/UI parity and external providers remain open.
...
- Added dedicated native block-list and block-details routes plus additive V4 persistent block parents. Block creation and rename are Admin-only and idempotent; flat membership follows a block rename without changing flat IDs, while duplicate names, unknown parents and cross-apartment IDs are rejected.
- Added dedicated native join-request list, review and rejection routes. The list now matches the reference states with Pending/Approved/Rejected filters; signup persists the selected Owner/Tenant type, and review shows the type and request date. Admin-only member details are tenant-scoped; rejection requires a persisted explanation, cannot be repeated, prevents account access and releases the claimed flat without deleting the rejected request.
- Installed the fresh debug APK on a Pixel_10_Pro Android 37 emulator and exercised the resident join flow against the retained local PostgreSQL backend. Tightened filter labels, shared request-card rows, date display and side-by-side review actions after direct frozen-reference comparison. The flow passed light/dark, portrait/landscape, validation, persisted rejection and Android Back checks; this is not whole-app or physical-device parity evidence.

## Deliberately not claimed
Not a complete 203-route pixel-identical native app, a whole-app/physical-device-validated APK or AAB, a verified fresh/V1-clone migration matrix, a configured OTP/FCM/payment integration, or a Google Play release. See status and evidence for the remaining acceptance gates.
