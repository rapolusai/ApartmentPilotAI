...
- Current backend JUnit test/package, Android clean debug build, PostgreSQL/Flyway V4 validation, 50-check local API suite and 111-check operations/concurrency suite passed. Device/UI parity and external providers remain open.
...
- Added dedicated native join-request list, review and rejection routes. Admin-only member details are tenant-scoped; rejection requires a persisted explanation, cannot be repeated, prevents account access and releases the claimed flat without deleting the rejected request.
