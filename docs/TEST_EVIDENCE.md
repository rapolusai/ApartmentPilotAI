...
| Operations API suite with concurrency | **111 checks passed** | Join-review/rejection authorization, persistence and flat release; Admin-only block creation/rename/idempotency and tenant isolation; affected flats, notices, contributions, transparency, files, bookings, polls, vehicles, subscriptions and parallel one-winner approval were exercised over HTTP. |
