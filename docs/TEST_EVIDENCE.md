...
| Operations API suite with concurrency | **100 checks passed** | Admin-only block creation/rename/idempotency, flat-parent enforcement and tenant isolation; staff-only affected-flat authorization; community, notice scheduling, contribution, transparency, files, bookings, polls, vehicles, subscriptions and parallel one-winner approval were exercised over HTTP. |
...
| Literal JDBC SQL argument inspection | **283 calls; zero placeholder mismatches** | Counts `?` parameters against source arguments. Dynamic SQL/varargs arrays excluded. New block persistence SQL was also exercised live; the static scan alone does not execute SQL. |
| Literal native navigation inspection | **102 targets; zero missing switch destinations** | Recalculated from current literal `go(...)`/`openPage(...)` calls and 152 route dispatch identifiers. Only literal source links; no Android navigation/runtime test. |
...
| Partial/merged subflow source, not a separately matched page | 57 |
| Not implemented as the reference route | 9 |
